<template>
	<div>
			...
			<van-cell-group>
				<van-cell v-if="hasRush && sourceType=='rusnNow'" title="结束时间">
					<van-tag plain type="danger">
						<count-down :time="endTime.replace(/\-/g, '/')" :callback="TimeOutCallback"></count-down>
					</van-tag>
				</van-cell>

				<van-panel title="销售价格" class="Sellprice" :status="formatRMBData(goodsItem)" />
		...
		<div class="sku-container">
			...
			<van-sku ref='goodsSkuData' v-model="showBase" :sku="skuData.sku" :goods="skuData.goods_info" :goods-id="skuData.goods_id"
			 :hide-stock="skuData.sku.hide_stock" :quota="skuData.quota" :quota-used="skuData.quota_used" reset-stepper-on-hide
			 reset-selected-sku-on-hide disable-stepper-input :close-on-click-overlay="closeOnClickOverlay"
			 :custom-sku-validator="customSkuValidator" @buy-clicked="onBuyClicked" :show-add-cart-btn="false">
				<!-- 表单商品图片和价格 -->
				<template slot="sku-header">
					<div class="van-sku-header van-hairline--bottom">
						...
						<div class="van-sku-header__goods-info">
							...
							<i @click="handleClose" class="van-icon van-icon-close van-sku__close-icon"></i>
						</div>
					</div>
				</template>
				<!-- 折扣劵 -->
						...
						<van-collapse-item title="规格" name="1">
							<van-radio-group v-model="skuForm.specId">
								<van-cell-group>
									...
									<van-cell v-for="(item, index) in goodsSpec" :title=" item.specList.toString() +' ￥' +item.price" :key="index"
              ...
              <div class="van-stepper van-sku__stepper">
				...
				<template slot="sku-messages">
					<van-cell-group class="user-group">
						<van-cell icon="location-o" title="收货地址" :value="ChangeTextByStatus()" is-link @click="modifyAddress" />
						<van-cell :title="defaultAddress" />
					</van-cell-group>
					<van-collapse value="1" accordion>
						<van-collapse-item title="填写表单" name="1">
							<van-row>
									 <van-col span="8">箱型</van-col>
									<van-col span="16">
										<input class="van-field__control" type="text"  v-model="skuForm.boxShape"  @click="boxFormulaShow=true" readonly="readonly"  placeholder="请选择箱型"/>
									</van-col>
							</van-row>
							
							
							 <van-field v-model="skuForm.yxType" clearable readonly maxlength="20" required label="压线类型" @focus="showXyType=true"
							  right-icon="question-o" placeholder="请选择压线类型" @click-right-icon="$toast('请选择压线类型')" />
								
							<van-field v-model="skuForm.boxLength" required clearable type='number' maxlength="11" label="箱长"
							 placeholder="请输入箱长" @click-right-icon="$toast('请输入箱长')" >
									<div slot="button">
										mm
									</div>
							 </van-field>
							 
							<van-field v-model="skuForm.boxWidth" required maxlength="20" clearable type='number' label="箱宽"
							  placeholder="请输入箱宽" @input="setPaperYx()" @click-right-icon="$toast('请输入箱宽')">
							 <div slot="button">
							 	mm
							 </div>
							 </van-field>
							 
							 <van-field v-model="skuForm.boxHeight" clearable type='number' maxlength="20" label="箱高" @input="setPaperYx()"
							  placeholder="请输入箱高" @click-right-icon="$toast('请输入箱高')" >
								 <div slot="button">
									mm
								</div>
								</van-field>
								
								<van-field v-model="skuForm.boxShapeType" clearable type='number' maxlength="2" label="成型方式"
								 right-icon="question-o" @click="boxShapeTypeShow=true" readonly placeholder="请选择成型方式" @click-right-icon="$toast('请选择成型方式')" />
							
								<van-field v-model="skuForm.paperLength" type='number' label="纸长" readonly />
								<van-field v-model="skuForm.paperWidth"  type='number' label="纸宽" readonly />
								
								<van-field v-model="skuForm.paperWidth"  type='text' label="纸长公式" readonly />
								<van-field v-model="skuForm.paperWidth"  type='text' label="纸宽公式" readonly />
								
							<van-field v-model="skuForm.paperNum" required maxlength="20" clearable type='number' label="数量(片)" right-icon="question-o"
							 placeholder="请输入数量" @click-right-icon="$toast('请输入数量')" />
							
							<van-field v-model="skuForm.paperYx" clearable maxlength="20" label="压线" right-icon="question-o" placeholder="请输入压线，格式 0+0+0"
							 @click-right-icon="$toast('计算公式：（纸宽 - 箱高)/2+箱高+（纸宽 - 箱高)/2')" />
							
							<van-actionsheet v-model="showXyType" :actions="yxTypeList" @select="onSelectXyType" />
							<van-field v-model="skuForm.deliveryRemark" clearable maxlength="60" label="送货备注" type="textarea" right-icon="question-o"
							 placeholder="请输入送货备注" @click-right-icon="$toast('请输入送货备注')" />
							<van-field v-model="skuForm.productionRemark" clearable maxlength="60" label="生产备注" type="textarea" right-icon="question-o"
							 placeholder="请输入生产备注" @click-right-icon="$toast('请输入生产备注')" />
						</van-collapse-item>
					</van-collapse>
		...
		<van-goods-action>
			<van-goods-action-mini-btn icon="wap-home" text="首 页" @click="goHome" />
			<van-goods-action-mini-btn icon="wap-nav" text="分 类" @click="onClickCategory" />
			<van-goods-action-mini-btn icon="contact" text="我的" @click="onClickUser" />
			<van-goods-action-big-btn primary :disabled="disabledBuy" text="立即购买" @click="purchase" />
		</van-goods-action>
		<van-popup v-model="boxFormulaShow" position="bottom">
			<van-picker :columns="boxColumns" show-toolbar @cancel="boxFormulaShow=false"/>
		</van-popup>
		<van-popup v-model="boxShapeTypeShow" position="bottom">
			<van-picker :columns="boxShapeTypeList" show-toolbar @cancel="boxShapeTypeShow=false"/>
		</van-popup>
	</div>
</template>
<script>
	...
	export default {
		...
		watch: {
			setTotalPrice: {
				handler: function(val, oldval) {
					//debugger
					if (this.goodsId != "" && this.skuForm.specId != "" && this.skuForm.paperLength != "" && this.skuForm.paperWidth !=
						"" && this.skuForm.paperNum != "") {
						this.getTotalPrice()
					}
		...
		computed: {
			...
			setPaperYxValue() {
				if (this.skuForm.boxHeight != "" && this.skuForm.paperWidth != "" && parseFloat(this.skuForm.boxHeight) <
					parseFloat(this.skuForm.paperWidth)) { //输入箱高之后需要计算压线
					//压线计算方式为 （纸宽 - 箱高)/2+箱高+（纸宽 - 箱高)/2
			...
			goodsSpec() {
				...
				let temp = [];
				this.boxFormulaList.forEach((item)=>{
					temp.push({key:item.id1,text:item.boxName});
				})
				return temp;
			}
		},
		...
		methods: {
			//添加修改地址文字提示
			ChangeTextByStatus() {
				if (this.defaultAddress.length > 10) {
					return '修改';
				} else {
					return '添加'
				}
			...
			formatRMBData(data) {
				...
				if (this.hasRush && this.sourceType == type.dataFrom_rusnNow) {
					...
					if (data != null && data.goodsPrice != null) {
						let goodsSpecList = JSON.parse(data.goodsSpec)
						if (goodsSpecList.length > 1) { //debugger
							return `¥：` + this.goodsPriceSort(goodsSpecList)
			...
			//规格价格排序,并返回第一个和最后一个价格
			goodsPriceSort(goodsSpecList) {
				let tempArray = [...goodsSpecList]
				let goodsSpecListSortByPrice = tempArray.sort(compareObj('price')); //根据对象的b属性的大小排序

				return `${goodsSpecListSortByPrice[0].price} - ${goodsSpecListSortByPrice[goodsSpecListSortByPrice.length-1].price} 元`;
			},
			//修改地址
			modifyAddress() {
			...
			checkInputData() {
				...
				if (this.skuForm.boxHeight != "" && this.skuForm.paperWidth != "" && parseFloat(this.skuForm.boxHeight) >
					parseFloat(this.skuForm.paperWidth)) {
					flag = false;
					this.$toast('箱高不能大于纸宽')
				}
			...
			//切换商品规格，改变价格
			swichPrice(data) {
				if (this.totalPrice != 0 && this.goodsId != "" && this.skuForm.specId != "" && this.skuForm.paperLength != "" &&
					this.skuForm.paperWidth != "" && this.skuForm.paperNum != "") {
					return this.totalPrice
				} else {
					this.totalPrice = 0
					//debugger
					if (this.goodsItem.goodsSpec != null && !this.hasRush && (this.skuForm.specId == null || this.skuForm.specId == "")) {
						let goodsSpecList = JSON.parse(this.goodsItem.goodsSpec)
						if (goodsSpecList && goodsSpecList.length > 1) {
							// console.log(this.goodsPriceSort(goodsSpecList))
							return this.goodsPriceSort(goodsSpecList)
						} else {
							return data
						}
				...
				return new Promise((resolve, reject) => {
					...
					this.$store.dispatch('getAddressList', params).then(res => {
						...
						if (addressDetail.length > 0) {
							_self.skuForm.deliveryAddress = addressDetail[0].id2;
							_self.defaultAddress = addressDetail[0].name + " " + addressDetail[0].phoneNo + " " + addressDetail[0].address;
						} else {
							_self.skuForm.deliveryAddress = _self.addressList[0].id2;
							_self.defaultAddress = _self.addressList[0].name + " " + _self.addressList[0].phoneNo + " " + _self.addressList[
								0].address;
						}
			...
			getBoxFormulaList() {
				this.$store.dispatch('getBoxFormulaList').then(res => {
					this.boxFormulaList = res;
				}).catch(err => {
		...
		created() {
			//debugger
			if (this.$store.getters.getTeamSku_state != null && this.$store.getters.getTeamSku_state.goodsId == this.$route.query
				.id &&
